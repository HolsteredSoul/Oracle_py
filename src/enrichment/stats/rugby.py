"""Rugby Union stats fetcher — ESPN (primary) + TheSportsDB fallback.

ESPN covers: Super Rugby Pacific, Premiership, Top 14, URC.
TheSportsDB fallback for unrecognised competitions (limited: 1 last event, no standings).
"""
...
def _fetch_via_thesportsdb(home: str, away: str) -> MatchStats | None:
    """Fallback — TheSportsDB (1 last event, no standings)."""
    home_team = _search_team_tsdb(home)
    away_team = _search_team_tsdb(away)

    if home_team is None and away_team is None:
        logger.info("Could not resolve either rugby team (TSDB): %s vs %s", home, away)
        return None

    stats = MatchStats(sport="rugby", home_team=home, away_team=away)
...
def _fetch_via_espn(home: str, away: str, league_key: str) -> MatchStats | None:
    """Rugby union via ESPN — standings + form from last 5 games."""
    stats = MatchStats(sport="rugby", home_team=home, away_team=away)
    ...
    logger.info(
        "ESPN rugby stats: %s v %s — completeness=%.0f%% (pos=%s/%s)",
...
def fetch_rugby_stats(
    ...
    """Fetch rugby union stats — ESPN for known leagues, TheSportsDB fallback."""
    ...
    if league_key is not None:
        ...
        if result is not None:
            return result
        logger.info("ESPN failed for %s v %s (%s), falling back to TheSportsDB",
                     home_canonical, away_canonical, league_key)
